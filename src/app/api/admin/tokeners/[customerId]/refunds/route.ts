...
import {
  createSupabaseAdminVendorRefund,
  supabaseAdminVendorRefundSchema,
...
export async function POST(
  ...
  try {
    ...
    const input = supabaseAdminVendorRefundSchema.parse({
      ...body,
      customerId,
    });

    return NextResponse.json(await createSupabaseAdminVendorRefund(input));
  } catch (error: unknown) {
    ...
    return NextResponse.json(
      {
        code: "INVALID_INPUT",
        message: "Refund could not be saved.",
      },
      { status: 400 },
    );
  }
