create or replace function public.vendor_charge_customer_wallet(
  p_customer_id uuid,
...
begin
  ...
  select coalesce(
    sum(
      case
        when direction = 'credit' then token_amount
        else -token_amount
      end
    ),
    0
  ...
  if v_customer_balance < p_token_amount then
    raise exception 'Customer wallet has insufficient tokens';
  end if;
  ...
  values (
    ...
    v_vendor_id,
    p_reference,
    'Vendor quick charge customer wallet debit.',
    p_occurred_at,
    'operation:' || p_idempotency_key,
    jsonb_build_object(
      'pairedLedgerEntryId', p_vendor_ledger_entry_id,
      'source', 'supabase_vendor_quick_charge'
    )
  );
  ...
  values (
    ...
    v_vendor_id,
    p_reference,
    'Vendor quick charge receipt.',
    p_occurred_at,
    'entry:vendor-credit:' || p_idempotency_key,
    jsonb_build_object(
  ...
  values (
    ...
    'ledger_entry',
    p_customer_ledger_entry_id,
    'Vendor quick charge recorded against customer wallet.',
    p_occurred_at,
    jsonb_build_object(
...
revoke all on function public.vendor_charge_customer_wallet(
  uuid,
  text,
  numeric,
  uuid,
...
grant execute on function public.vendor_charge_customer_wallet(
  uuid,
  text,
  numeric,
  uuid,
