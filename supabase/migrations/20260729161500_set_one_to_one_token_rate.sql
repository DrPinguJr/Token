...
alter table public.settlements
  alter column earned_token_amount type numeric(14, 2)
  using earned_token_amount::numeric(14, 2);
...
begin
  ...
  if v_tokens_per_dollar is null then
    raise exception 'Event token rate is not configured';
  end if;

  if v_tokens_per_dollar <> 1 then
    raise exception 'Event token rate must be one token per dollar';
  end if;
